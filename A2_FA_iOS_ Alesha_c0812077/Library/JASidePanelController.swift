...
public class JASidePanelController: UIViewController,UIGestureRecognizerDelegate {
    ...
    public var leftPanel: UIViewController!  {
        set{
            if newValue != _leftPanel {
                if(_leftPanel != nil){
                    _leftPanel.willMove(toParent: nil)
                    _leftPanel.view.removeFromSuperview()
                    _leftPanel.removeFromParent()
                }
                self._leftPanel = newValue
                if (_leftPanel != nil) {
                    self.addChild(_leftPanel)
                    _leftPanel.didMove(toParent: self)
    ...
    public var rightPanel: UIViewController!{
        set{
            if newValue != _rightPanel {
                if(_rightPanel != nil){
                    _rightPanel.willMove(toParent: nil)
                    _rightPanel.view.removeFromSuperview()
                    _rightPanel.removeFromParent()
                }
                self._rightPanel = newValue
                if (_rightPanel != nil) {
                    self.addChild(_rightPanel)
                    _rightPanel.didMove(toParent: self)
                }
    ...
    func leftButtonForCenterPanel() -> UIBarButtonItem {
        if (menuImage == nil) {
            ...
            return UIBarButtonItem(image: menuImage, style: .plain, target: self, action: #selector(self.toggleLeftPanel))
        }
